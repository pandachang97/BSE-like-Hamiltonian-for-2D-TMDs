...
def coulomb_rpa_kernel_flexible(
    *,
    ...
    AINV_TO_BOHRINV = 1.889726124565062 # Å^-1 → Bohr^-1
    ...
    if not chi_is_inverse:
        ...
        def denom_factor(k_bohr: float) -> float:
            return 1.0 + 2.0  * chi_len_bohr * k_bohr
    else:
    ...
    def kernel(i: int, j: int, items: Sequence[KPItem]) -> float:
        ...
        W_hartree = (2.0 ) / denom          # e^2 = 1 in a.u.
...
def build_bse_matrix_with_phase(
    ...
    *,
    upper_uses_conjugate: bool = False,  # False → K_plus, True → K_minus
) -> ArrayC:
    """
    ...
    for i in range(N):
        for j in range(i+1, N):
            Wij = complex(kernel(i, j, items))      # real or complex kernel
            dphi = phi[i] - phi[j]                   # Δφ_ij
            phase = np.exp(-1j * dphi)              # e^{-iΔφ}
            Zij = Wij * phase
            if upper_uses_conjugate:
                # K_minus behavior
                H[i, j] = np.conjugate(Zij)
                H[j, i] = Zij
            else:
                # K_plus behavior
                H[i, j] = Zij
                H[j, i] = np.conjugate(Zij)
    return H
...
def build_bse_two_valleys_from_series(
    ...
    *,
    ...
    phi_plus, phi_minus = phi_angles_two_valleys(series)

    # K_plus: H[i,j]=W; H[j,i]=W*
    Hp = build_bse_matrix_with_phase(plus_items,  kernel_plus, phi_plus,  upper_uses_conjugate=False)
    # K_minus: H[i,j]=W*; H[j,i]=W
    Hm = build_bse_matrix_with_phase(minus_items, kernel_minus, phi_minus, upper_uses_conjugate=True)
    ...
    return {
        "K_plus":  BSEOutput(H_BSE=Hp, gaps=gaps_p, k_grid=Kp),
        "K_minus": BSEOutput(H_BSE=Hm, gaps=gaps_m, k_grid=Km),
    }
